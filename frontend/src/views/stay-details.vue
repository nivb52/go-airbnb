<template>
  <div>
    <nav-details
      :stay="stay"
      :isReserveNavShown="isReserveNavShown"
      v-if="isDetailsNavShown && stay"
    ></nav-details>
    <div v-if="stay" class="stay-details detailsContainer">
      ...
      <div class="detailsColumnsContainer">
        <div class="column1">
          ...
          <div class="aircover">
            <img
              src="https://res.cloudinary.com/dgvpl7cdq/image/upload/v1674293987/czcblqj1okmht5xmeuq3.png"
              alt=""
            />
            <p>
        ...
        <reserve-form
          :class="{ openedReserve: IsOpenedReserveMobile }"
          :stay="stay"
        ></reserve-form>
      </div>
...
export default {
  ...
  async created() {
    await this.setStay();
    if (this.rating > 4.6 && this.stay.reviews.length > 3)
  ...
  methods: {
      ...
      this.Observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (
              entry.target ===
              document.querySelector('.stay-details-imgs-container')
            ) {
    ...
    async setStay() {
      ...
      if (stayId) {
        const currStay = await this.$store.dispatch({
          type: 'setCurrStay',
          stayId,
        });
